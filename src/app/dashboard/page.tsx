...
export default function DashboardPage() {
  ...
  const [sendingBriefing, setSendingBriefing] = useState<
  ...
  // Collapsible relationship section — auto-expands if at-risk contacts exist
  const [relationshipExpanded, setRelationshipExpanded] =
    ...
    try {
      ...
      setError(
        (err instanceof Error ? err.message : String(err)) ||
          "Failed to load briefing dashboard data.",
      );
    } finally {
      setLoading(false);
    }
  ...
  const fetchExecutiveBriefing = useCallback(async (force = false) => {
    ...
    try {
      ...
      if (!res.ok) {
        const errorData = await res.json();
        throw new Error(
          errorData.error || "Failed to generate executive briefing",
        );
      }
      const briefingData = await res.json();
  ...
  // Auto-expand relationship section when at-risk contacts are detected
  useEffect(() => {
    if (
      data?.relationships?.atRiskCount &&
  ...
  const handleCompleteAction = async (
    ...
    if (briefing) {
      setBriefing({
        ...briefing,
        recommendedActions: briefing.recommendedActions.filter(
          (a) => a.id !== actionId,
        ),
      });
    }
  ...
  const updateStatus = async (id: string, newStatus: CommitmentStatus) => {
    ...
    setData((prev) => {
      ...
      return {
        ...
        lists: {
          ...
          highConfidenceCommitments: updateList(
            prev.lists.highConfidenceCommitments,
          ),
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(
          errData.error || `Failed to update status to ${newStatus}`,
        );
      }
      ...
      const message =
        err instanceof Error
          ? err instanceof Error
            ? err.message
            : String(err)
          : "Failed to save commitment status.";
      console.error(
        `Failed to update commitment ${id} to status ${newStatus}:`,
        err,
      );
      setError(message);
  ...
  const handleManualSync = async () => {
    ...
    try {
      ...
      if (res.ok) {
        await Promise.all([fetchBriefing(false), fetchExecutiveBriefing(true)]);
      }
    } catch (err) {
      ...
      >
        ...
        <header className="p-6 md:p-8 border-b border-[#E8ECF0] flex flex-col md:flex-row md:items-center justify-between gap-4 shrink-0 relative z-10">
          <div>
            ...
            <p className="text-[#57534E] text-sm mt-1.5 font-medium">
              {formatBriefingDate()}
            </p>
          </div>
        ...
        <div className="p-6 md:p-8 space-y-8 relative z-10 flex-1">
            ...
            {loading ? (
              ...
              <>
                ...
                <SpotlightCard className="p-5 flex flex-col justify-between group h-full">
                  ...
                  <span className="text-[#57534E] text-[10px] font-bold uppercase tracking-widest">
                    Pending Promises
                  </span>
                  <span className="text-3xl font-black mt-4 text-amber-400">
                    {data.stats.pendingCount}
                  </span>
                  <p className="text-[10px] text-[#57534E] mt-2">
                    Active obligations in progress
                  </p>
                </SpotlightCard>
                ...
                >
                  ...
                  <span className="text-3xl font-black mt-4 text-emerald-400">
                    {data.stats.completedCount}
                  </span>
                  <p className="text-[10px] text-[#57534E] mt-2">
                    Successfully closed out obligations
                  </p>
                </SpotlightCard>
                ...
                >
                  ...
                  <span
                    className={`text-3xl font-black mt-4 ${data.stats.overdueCount > 0 ? "text-rose-400" : "text-slate-350"}`}
                  >
                    {data.stats.overdueCount}
                  </span>
                  <p
                    className={`text-[10px] mt-2 font-medium ${data.stats.overdueCount > 0 ? "text-rose-400" : "text-[#57534E]"}`}
                  >
                    {data.stats.overdueCount > 0
                ...
                >
                  ...
                  <p className="text-[10px] text-[#57534E] mt-2">
                    Active threads awaiting review
                  </p>
                </SpotlightCard>

                {/* Upcoming Meetings Stat */}
                <SpotlightCard
                  className="p-5 flex flex-col justify-between group h-full col-span-2 lg:col-span-1"
                ...
                >
                  ...
                  <div className="mt-4 flex flex-col gap-1">
                    <span className="text-3xl font-black text-sky-400">
                      {data.calendarStats?.todayEventCount || 0}
                    </span>
                  ...
                  <p className="text-[10px] mt-2">
                    {(data.calendarStats?.conflictCount ?? 0) > 0 ? (
                      <span className="text-amber-400 font-bold">
                        {data.calendarStats?.conflictCount} conflicts detected.
                      </span>
                    ) : (
                      <span className="text-[#57534E]">
                        Your schedule is clear of conflicts
                      </span>
                    )}
                  </p>
                </SpotlightCard>
          ...
          {/* ── 2-Column Layout: Briefing + Calendar Sidebar ── */}
          {data?.calendarStats && (
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 items-start">
              {/* Calendar Today Panel (col 3) */}
              <div className="xl:col-start-3">
                <div className="bg-white border border-[#E8ECF0] shadow-sm rounded-2xl p-5">
                  <div className="flex items-center gap-2 mb-4">
                    <div className="h-6 w-6 rounded-lg bg-sky-100 border border-sky-200 flex items-center justify-center">
                      <svg
                    ...
                    <a
                      href="/calendar"
                      className="ml-auto text-[10px] font-bold text-[#A16207] hover:underline"
                    >
                      View all →
                    </a>
                  </div>
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className="text-3xl font-black text-sky-500">
                        {data.calendarStats.todayEventCount}
                      </span>
                      <span className="text-xs text-[#57534E] font-medium">
                        meeting
                        {data.calendarStats.todayEventCount !== 1 ? "s" : ""}{" "}
                        today
                      </span>
                    </div>
                    {data.calendarStats.nextEvent && (
                      <div className="p-3 rounded-xl bg-sky-50 border border-sky-100">
                        <p className="text-[9px] font-extrabold uppercase tracking-widest text-sky-500 mb-1">
                          Next Up
                        </p>
                        <p className="text-xs font-semibold text-[#0C0A09] truncate">
                          {data.calendarStats.nextEvent.title}
                        </p>
                        <p className="text-[10px] text-[#57534E] mt-0.5">
                          {new Date(
                            data.calendarStats.nextEvent.startAt,
                          ).toLocaleTimeString([], {
                            hour: "2-digit",
                            minute: "2-digit",
                          })}
                          {data.calendarStats.nextEvent.attendeeCount > 0 &&
                            ` · ${data.calendarStats.nextEvent.attendeeCount} attendees`}
                        </p>
                      </div>
                    )}
                    {data.calendarStats.upcomingWeekCount > 0 && (
                      <p className="text-[10px] text-[#57534E]">
                        <span className="font-bold text-[#0C0A09]">
                          {data.calendarStats.upcomingWeekCount}
                        </span>{" "}
                        more this week
                        {(data.calendarStats.conflictCount ?? 0) > 0 && (
                          <span className="ml-2 text-amber-500 font-bold">
                            · {data.calendarStats.conflictCount} conflict
                            {data.calendarStats.conflictCount !== 1 ? "s" : ""}
                          </span>
                        )}
                      </p>
                    )}
          ...
          {!loading &&
            ...
            data.relationships.totalContacts > 0 && (
              <div className="rounded-2xl border border-[#E8ECF0] overflow-hidden bg-white shadow-sm">
                {/* Accordion Header */}
                <button
                  onClick={() => setRelationshipExpanded((v) => !v)}
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      ...
                      <p className="text-[10px] text-[#57534E]">
                        {data.relationships.strongCount} strong ·{" "}
                        {data.relationships.neutralCount} neutral
                        {data.relationships.atRiskCount > 0 && (
                          <span className="text-rose-500 font-bold ml-1">
                            · {data.relationships.atRiskCount} at risk ⚠
                          </span>
                        )}
                      </p>
                    </div>
                  ...
                  <div className="flex items-center gap-3">
                    <div className="hidden sm:flex h-2 w-24 rounded-full overflow-hidden bg-[#E8ECF0]">
                      {data.relationships.strongCount > 0 && (
                        <div
                          className="bg-emerald-500"
                          style={{
                            width: `${(data.relationships.strongCount / data.relationships.totalContacts) * 100}%`,
                          }}
                        />
                      )}
                      {data.relationships.neutralCount > 0 && (
                        <div
                          ...
                          style={{
                            width: `${(data.relationships.neutralCount / data.relationships.totalContacts) * 100}%`,
                          }}
                        />
                      )}
                      {data.relationships.atRiskCount > 0 && (
                        <div
                          ...
                          style={{
                            width: `${(data.relationships.atRiskCount / data.relationships.totalContacts) * 100}%`,
                          }}
                        />
                      )}
                    </div>
                    <a
                      ...
                      className="text-[10px] font-bold text-[#A16207] hover:underline hidden sm:block"
                    >
                      View all →
                    </a>
                    <svg
                      className={`w-4 h-4 text-[#57534E] transition-transform duration-200 ${relationshipExpanded ? "rotate-180" : ""}`}
                      fill="none"
                      viewBox="0 0 24 24"
                      stroke="currentColor"
                    >
                ...
                <AnimatePresence>
                  {relationshipExpanded && (
                    ...
                    >
                      <div className="px-6 pb-6 space-y-6 border-t border-[#E8ECF0]">
                        {/* Health Distribution */}
                        <div className="pt-5 grid grid-cols-3 gap-4">
                          <div className="p-4 rounded-xl bg-emerald-50 border border-emerald-100 text-center">
                            <span className="text-2xl font-black text-emerald-400">
                              {data.relationships.strongCount}
                            </span>
                          ...
                          <div className="p-4 rounded-xl bg-white border border-[#E8ECF0] text-center">
                            <span className="text-2xl font-black text-slate-600">
                              {data.relationships.neutralCount}
                            </span>
                            <div className="flex items-center justify-center gap-1.5 mt-1">
                        ...
                        {/* Top + At-Risk side by side */}
                        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
                          <div>
                            <p className="text-[9px] font-extrabold uppercase tracking-widest text-[#57534E] mb-3">
                              Top Relationships
                            </p>
                            <div className="space-y-3">
                              {data.relationships.topContacts.length === 0 ? (
                                <p className="text-[11px] text-[#57534E] text-center py-6">
                                  No contacts scored yet.
                                </p>
                              ) : (
                          ...
                          <div>
                            <p className="text-[9px] font-extrabold uppercase tracking-widest text-rose-500 mb-3">
                              Needs Attention
                            </p>
                            <div className="space-y-3">
                              {data.relationships.atRiskContacts.length ===
                              0 ? (
                                <p className="text-[11px] text-[#57534E] text-center py-6">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 items-start">
              ...
              {loading ? (
                ...
                data.lists.upcomingDeadlines.map((c) => (
                  <BriefingItemCard
                    ...
                    onComplete={(id) =>
                      updateStatus(id, CommitmentStatus.COMPLETED)
                    }
                    onSnooze={(id) =>
                      updateStatus(id, CommitmentStatus.SNOOZED)
                    }
            ...
            <FocusColumn
              title="Obligations Map (High Confidence)"
              icon={
                <svg
            ...
            <FocusColumn
              title="Recent Extraction Insights"
              icon={
                <svg
                  className="w-4 h-4 text-amber-400"
                  fill="none"
...
function BriefingItemCard({
  ...
  return (
    <div
      className={`p-4 rounded-xl border border-[#E8ECF0] transition-all flex flex-col gap-3 group relative overflow-hidden ${cardBorderClass}`}
    >
      {/* Badges / Meta row */}
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {commitment.dueDate && (showDate || isOverdue) && (
          <span
            className={`text-[9px] font-bold ${isOverdue ? "text-rose-450" : "text-slate-450"}`}
          >
      ...
      {isPending &&
        ...
        commitment.riskReason !== "No significant risk factors active." && (
          <div className="text-[10px] text-[#57534E] bg-slate-950/50 border border-[#E8ECF0] rounded-lg p-2 flex items-start gap-1.5">
            <span className="text-amber-500 shrink-0 text-[11px] leading-none">
              ⚠️
            </span>
      ...
      {commitment.sourceEmail && (
        <div className="text-[10px] text-[#57534E] border-t border-[#E8ECF0] pt-2.5 flex flex-col gap-0.5">
          ...
          <span className="truncate">
            {commitment.direction === "INBOUND" ? "Sender" : "Recipient"}:{" "}
            {formatSender(commitment.sourceEmail.sender)}
          </span>
        </div>
      )}
      ...
      {!isPending && (
        <span
          className={`text-[9px] font-bold self-start px-2 py-0.5 rounded border mt-1 ${
            isCompleted
  ...
  return (
    <div className="p-3.5 rounded-xl border border-[#E8ECF0]/50 bg-[#0c0f1f]/30 hover:bg-[#F3F4F6] hover:border-[#E8ECF0]/50 transition-all group">
      <div className="flex items-start gap-3">
        ...
        <div
          className={`h-9 w-9 rounded-full border-2 ${ringColor} flex items-center justify-center font-bold text-xs text-[#0C0A09] bg-[#FAFAF9]/80 shrink-0`}
        >
          {initial}
        </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between gap-2">
            <div className="truncate">
              ...
              <span className="text-[10px] text-[#57534E] block truncate">
                {contact.email}
              </span>
            </div>
          ...
          <div className="flex items-center gap-3 mt-2 text-[10px] text-[#57534E] font-medium">
            ...
            {contact.openCommitments > 0 && (
              <span className="text-amber-400">
                {contact.openCommitments} open
              </span>
            )}
            {contact.completedCommitments > 0 && (
              <span className="text-emerald-400">
                {contact.completedCommitments} done
              </span>
            )}
          </div>
...
export function ExecutiveBriefingWidget({
  ...
  return (
    <SpotlightCard
      className="p-6 relative overflow-hidden shadow-2xl"
      spotlightColor="rgba(167, 139, 250, 0.15)"
    >
                ...
                briefing.recommendedActions.map((action) => (
                  ...
                  >
                    <div className="flex items-center gap-3 min-w-0">
                      ...
                      >
                        <svg
                          className="w-3.5 h-3.5 text-transparent hover:text-emerald-400 transition-colors"
                          fill="none"
                          viewBox="0 0 24 24"
                          stroke="currentColor"
                        >
                ...
                briefing.topRisks.map((risk) => (
                  ...
                  >
                    <div className="flex items-center justify-between gap-2">
                      ...
                      <span
                        className={`text-[8px] font-extrabold px-1 rounded-md ${
                          risk.riskLevel === "HIGH"
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="space-y-3">
            ...
            {[1, 2].map((i) => (
              <div
                key={i}
                className="h-12 w-full bg-[#F3F4F6]/40 border border-[#E8ECF0] rounded-xl"
              ></div>
            ))}
          </div>
        </div>
